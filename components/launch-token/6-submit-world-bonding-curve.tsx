...
import { TITLE_MAP } from "./4-submit-world";
...
export const SubmitWorldBondingCurve = () => {
      ...
      reader.onloadend = async () => {
        if (typeof reader.result === "string") {
          ...
          try {
            ...
            setValue("image", res.fileUrl, { shouldValidate: true });
            setImageMetadata({
  ...
  const resetFormBanner = (shouldValidate = false) => {
    ...
    setValue("bannerUrl", "", { shouldValidate });
  };
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        if (typeof reader.result === "string") {
          ...
          try {
            const res = await _uploadFile(file);
            setValue("bannerUrl", res.fileUrl, { shouldValidate: true });
            setBannerMetadata({
              size: file.size / 1024 / 1024,
              name: file.name,
            });
  ...
  const onSubmit = (data: z.infer<typeof formSubmitWorldSchema>) => {
    const submitData = {
      ...
      fundraisingType: "bonding-curve" as const,
    };
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="z-70 w-20 h-20 relative">
            <div
              className={cn(
            ...
            >
              ...
              {isLoadingImage && <Skeleton className="w-20 h-20" />}
            </div>
            <div
              className={cn(
                "z-50 absolute top-0.5 left-0.5",
        ...
        >
          <div className="relative z-20 flex flex-col gap-1 md:basis-full">
            <Text
              variant="small"
              className="text-[#ededed] uppercase tracking-wide"
            >
              Optional
            </Text>
          </div>
        ...
        <motion.div variants={itemVariants}>
          <FormWithLabel label="Token Name" required>
            <InputController
              ...
              maxLength={30}
              append={
                <Text className="text-grey-300">{(name || "").length}/30</Text>
              }
            />
          </FormWithLabel>
        </motion.div>
          ...
          <FormWithLabel
            ...
            append={
              <motion.div
                ...
                animate={openExtension ? "open" : "closed"}
                transition={{ duration: 0.3 }}
              >
                <ChevronDownIcon
